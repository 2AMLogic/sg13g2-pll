...
| Solver | ngspice defaults | **`itl4=5000`** on the four transient templates | Two measured hard aborts at `mos_ff`/`res_wcs`/−40 °C/24.4 MHz. Not an accuracy relaxation; see the SOLVER-EFFORT NOTE in `../testbench/tb_hyst_diag.sp.tmpl` for the failing points, the rejected alternatives and the measured numerical impact |
