...
deliberate — issue #66 fixes the same two-net feedback defect in the shared
cell on both PDKs, and "measured on both" is a stronger claim than "measured
on one and argued across". It is filed under the CMOS5L slug rather than
opening an SG13G2 slug for two nets, and it does not constitute an SG13G2
campaign: the SG13G2 `lock_detector`'s own `XRPU`/`XCW`/`XMPD` are still at
their pre-#52 values and nothing here measures them.

## SG13CMOS5L campaign status

Tracks issues #23, #27, #30, #36, #37, #38, #52 and #66 (all Part of #16). #23 owns the
...
last measurable criterion. **#30 is the first campaign here that measures a
*layout* rather than a schematic**: it re-runs two of the matrices below
against parasitic-annotated netlists extracted from the routed GDS, each
alongside a schematic-level control arm re-run on the same host, and is the
named follow-up issue #24 deferred post-layout PVT to. It adds **no new spec
row and amends none** — it re-measures rows already bounded above and reports
how far layout moves them. The table below is the whole campaign, in the
...
| [`sg13cmos5l-closed-loop-lock`](sg13cmos5l-closed-loop-lock/) | Transistor-level closed loop (`pfd`+`cp`+`loop_filter`+`vco`+`divider_chain`+`lock_detector`, testbench-local wiring — no `pll_top` schematic exists), single PVT point (runtime-cost subset, see that record's own `corners/matrix.md`) | 7 (lock time), 10 (reference spur), 11 (power, remaining domains) | **Row 11 bounded** (with caveats): all five remaining domains measured simultaneously at one operating point — `pfd`+`cp`+`vco`+`lock_detector` = 2.483 mA (8.20 mW); `divider_chain` alone = 7.653 mA (25.26 mW), but flagged as very likely inflated by that block's own known malfunction (see below), not a clean design figure. **Rows 7 and 10 stay `insufficient-evidence`**, but the `pfd` self-reset inverter-parity defect `records/RECORD-002` (issue #50) root-caused is now **fixed and re-verified** (`records/RECORD-003`, issue #56): with the corrected `pfd`, the proposal loop (Part B) achieves genuine **frequency lock** (`Δf`/`f_ref` within 0.03% of `f_ref` for a full microsecond, zero cycle slips — vs. `RECORD-001`'s own monotonically-diverging ≈23% `Δf`) but settles at a stable **static phase error of ≈9.18% of a reference period**, ~1.8× this record's own 5% row-7 threshold, so the dual lock criterion is still not met. **`records/RECORD-004` (issue #70) then confirms the mechanism**: an ideal, diagnostic-only substitution forcing `cp`'s `up`/`dn` currents to be exactly equal (NOT a change to the committed `cp`) collapses the static phase error to a mean of −0.00235% (from 9.176%, `RECORD-003`'s own final-20-cycle window) and to simulator floating-point noise in the fully-settled tail, while also extending the longest continuous dual-lock run from 4 cycles to 41. RECORD-004's own decision: this is a design-level `cp` current-mirror-matching defect worth mitigating, not an acceptable residual — filed as issue #72. The as-drawn loop (Part A) still does not lock, independently over-determined by the separate `divider_chain` defect (#36). **`records/RECORD-005` (issue #72) then implements and re-runs the real mitigation** (`design/sg13cmos5l/cp.sch` gains an on-chip high-swing cascode bias replica, `spec/decision-records/DR-006`): the DC-measured up/dn mismatch this mitigation targets drops 20x at the loop's own operating point (`sim/sg13cmos5l-cp-icp-trim/records/RECORD-002`: −6.42% → −0.298%), but the closed-loop static phase error only drops from 9.176% to **8.203%** — an ~11% relative reduction, not the ~3,900x collapse RECORD-004's idealized diagnostic showed for the same nominal mechanism — and the longest continuous dual-lock run stays at 4 cycles. **Row 7 still fails for the mitigated design, not just the as-drawn one.** RECORD-005's own analysis: RECORD-004's ideal substitution likely also removed switching/dynamic charge-mismatch effects (already flagged as unmeasured by `sim/sg13cmos5l-cp-icp-trim/RECORD-002`) alongside the DC magnitude term it was built to isolate, and that dynamic term — not DC magnitude, which is now small — is the best-fitting remaining candidate for row 7's residual, though not itself confirmed by a dedicated diagnostic. **This slug's own testbench also hosts `run_closed_loop_vwin_dwell.sh` (issue #76)**, which instruments both `lock_detector` revisions side by side inside a closed-loop acquisition to answer a `sg13cmos5l-lock-detector-window` question (whether `schmitt_hv`'s crowbar current is reachable in real operation, not a claim this slug's own rows track) — see that slug's `records/RECORD-004-crowbar-current-mitigation.md` |
| [`sg13cmos5l-lock-detector-window`](sg13cmos5l-lock-detector-window/) | Real-subckt `lock_detector` assert window, hysteresis, chatter and supply current, plus the `XCW`/`XDW.XC1` MOM-uncertainty sensitivity | 16 (lock-detector targets), 11 (power, one more domain) | **`RECORD-001` (#38, pre-resize): window, hysteresis and no-chatter all bounded, and all three fail the ported criteria at every corner.** Window = 0.219–0.409 ns (target ≥2.5 ns); no hysteresis resolves at the ladder's 0.15×-window step at any of 92 points (target ≥25% of window); **92/92 points chatter**, including at a 10×-window static phase error. Cause is measured directly: the integrating node's own `XRPU`·`XCW` time constant (0.71–1.71 ns) is 23–1412× shorter than the reference period across the whole ported 1–25 MHz range, so `VWIN` fully re-settles every cycle regardless of phase-error size. The ±20% MOM band moves the window 7–8% (not the dominant term) and does not change the chatter/no-hysteresis verdict at any point. Static-phase-offset comparison stays `insufficient-evidence` (needs an unmeasured PFD/CP record); `lock_detector`'s own supply current (0.79–60.3 µA) is bounded, corroborating the combined figure in the `sg13cmos5l-closed-loop-lock` row above with a dedicated per-domain measurement.<br><br>**`RECORD-002` (#52, post-resize): `XRPU` `l=6u`→`l=700u`, `XCW` `w=8u l=8u`→`w=40u l=40u`, `XDW.XC1` `w=4u l=4u`→`w=40u l=40u` — the window and no-chatter criteria now PASS.** Window = 3.688–11.24 ns against the ≥2.5 ns floor at 81/81 points (worst-case margin 1.475×, including an explicit worst-case axis stack RECORD-001's grid did not contain); `steady` at 18/18 ladder corners at a 10×-window static phase error, at both ends of row 2's DR-005-amended 3.5–24.4 MHz range; `R·C` = 2.29–5.58 µs = **8.0–19.5×** the slowest reference period (6.4–23.4× including the ±20% MOM band), against 23–1412× *below* it before. **Hysteresis still fails** (<20% of window vs ≥25%), and RECORD-002 separates the cause into two measured terms that are both outside the three devices #52 resized: `schmitt_hv`'s feedback devices are tied to the wrong rails (0.9–1.6 mV measured, vs 881–979 mV for the classic connection) and — binding over that — the settled-`VWIN`-vs-phase-error transition is ≤0.05× the window wide, set by the `XRPU`/`XMPD` strength ratio, so rewiring the Schmitt alone measurably changes nothing. Row 11's `lock_detector` domain re-bounded at 2.48–95.1 µA for the resized block. One fixed sizing covers the whole amended `f_ref` range, so **row 2 needs no narrowing and no decision record is owed**.<br><br>**`RECORD-003` (#66, post-hysteresis-fix): `schmitt_hv`'s two feedback devices re-tied to the classic connection and `XMPD` `w=2u l=0.5u`→`w=0.25u l=16u` — ALL THREE of row 16's measurable criteria now PASS.** Hysteresis **50–800% of the window, 0 of 21 corners below the ≥25% criterion** (worst case 50%, i.e. 2.0× the criterion, at the 24.4 MHz corners); `steady` at **21/21** at a **20×**-window static phase error (RECORD-002 probed 10×); assert window **3.688–11.24 ns at 102/102** points and `R·C` **byte-identical** to RECORD-002 at 8.0–19.5× `T_ref`, so #52's two passing criteria are re-measured rather than assumed. The two fixes are each necessary and neither is sufficient: `schmitt_hv` goes from **0.88–1.58 mV** to **804 mV–1.058 V** of input-referred hysteresis (measured on **both** PDKs, 108 rows), and `XMPD`'s size is a **two-sided measured bound** — stronger fails the criterion at the fast end, weaker pushes the de-assert threshold past two-thirds of a reference period at the slow end. This record also runs on **x86-64**, so `cap_cmomi.osdi` loads and the `real` DUT variant returns: it closes RECORD-002's own arm64 gap (the closed-form nominal matches the real model to every printed digit; `real` vs. `ideal0.00` moves `twin_r` by ≤0.057%). Row 11's `lock_detector` domain re-bounds to **2.47–234 µA**, the top of which is attributed by control to `schmitt_hv` crowbar current while `VWIN` rests inside the now-wide hysteresis band, not to switching<br><br>**`RECORD-004` (#76, post-crowbar-mitigation): decided by measurement that the crowbar current is an accepted residual in realistic operation, then mitigated anyway.** A `sg13cmos5l-closed-loop-lock` deck (`run_closed_loop_vwin_dwell.sh`) instruments the pre-mitigation block inside a genuine closed-loop acquisition at three initial control voltages, now representative since issue #72's `cp` mismatch closed via #85: **`VWIN` never enters the hysteresis band** (`dwell_frac` = 0.000000 at all three `VC0` points; the pre-#52 control block reaches 0.0001 at one). `schmitt_hv`'s six channel lengths are lengthened `l=0.5u`→`l=2u` regardless (crowbar current 3.2–4.3× lower at every corner, `../sg13cmos5l-lock-detector-window/corners/schmitt_crowbar_variants.csv`, 180 rows; hysteresis-in-volts changes only 0.978–1.036×; the static in-band phase-error zone this cell reaches is unchanged at every corner to within ~1.4 percentage points of `T_ref`) and landed on the SG13G2 sibling `schmitt_hv.sch` in this same change. **All three of row 16's measurable criteria re-verified, unchanged in substance**; row 11's `lock_detector` domain re-bounds to **2.48–113 µA** (10×-window probe, down from 234 µA) / 40.75–103.29 µA (20×-window probe, now the full 21-corner matrix rather than one spot-checked corner) |
...
| [`sg13cmos5l-postlayout-pex-pvt`](sg13cmos5l-postlayout-pex-pvt/) | **Post-layout**: the `vco` and `cp` campaigns above re-run against parasitic-annotated netlists `klt extract --deck sg13cmos5l --parasitics` produced from the ROUTED layout, each alongside a schematic-level control arm re-run on the same host | 4/5 (Kvco), 6/6a (Icp trim) -- re-measured post-layout, not re-derived | **Real extracted parasitics WERE modelled** -- stated in as many words, per issue #30's own AC: klt's curated `sg13cmos5l` metal R/C coefficients (klayout-tools#2126, merged 2026-09-19, closing #2113), with `metals_without_coefficient` EMPTY for all six routed blocks. Extraction re-run from a clean upstream checkout and reproduces the committed netlists byte-identically. **`cp` (LVS match, 20/20 devices): the ratified Icp-trim table survives post-layout unchanged** -- +0.055..+0.632% (UP) / +0.017..+0.211% (DN) over 306 PVT points, worst case 0.51 uA on a 2.5-80 uA ladder. **`vco`: 0.4768-0.5312x the schematic frequency at 60/60 points** (band 223.7-789.5 MHz vs. 445.3-1562.0 MHz), attributed by a 7-variant diagnostic to **~99% parasitic CAPACITANCE** (extracted device geometry alone: 0.69%; parasitic R: a further ~0.7 pp). **Three caveats the record states rather than implies**: (a) that 2x is an UPPER BOUND set by routing style, not a floorplan prediction -- this flow draws 7178 um of wire for a ~45-device ring VCO, 413 um on `ring1` alone, and a C-scaling bracket puts a 10x-less-wire floorplan near -6..-10%; (b) `vco` has NO confirmed layout-vs-schematic topology match, so part of its deviation could be the unmatched comparison (bounded, not root-caused); (c) the deck's coefficients are the uncalibrated public-data starter values klayout-tools' own docstring says they are. Both control arms reproduce their committed campaigns (VCO 60/60 byte-identical; `cp` to 4.9e-5%). **No spec row is amended** on a non-representative floorplan |
...
pass at every corner re-measured, with the hysteresis criterion still
failing and its cause separated into two measured terms outside that
resize's own scope; (#66) then fixes both of those terms and re-measures
again — **all three of row 16's measurable criteria now pass**, hysteresis
at 50–800% of the window against a ≥25% target with 0 of 21 corners below
it. The "≥ 2× worst static phase offset" half of the row stays
`insufficient-evidence` (see the campaign table above). Row 11 (power) is likewise no longer deferred as a whole: every
...
Tracks issues #81, #82 (Part of #16 via #78), #97 and #120. #81/#82 are
Phases 1/2 and 2/2 of the decomposition #78's own Scope section calls for
("stand up the slug + extract R/C" then "re-derive and re-measure"); both
have landed. #97 is a second, independent SG13G2 slug — the SG13G2 twin of
the SG13CMOS5L port's own `sg13cmos5l-vco-kvco-table` (issue #23). #120 is
the third: re-verification of the #112 divider repair ported to the
non-port `design/` tree.
...
| [`sg13g2-vco-kvco-table`](sg13g2-vco-kvco-table/) | Open-loop SG13G2 `vco` frequency vs. `VCTRL` vs. 2-bit band code, real transient sweep (`XCDECAP`/`cap_cmim` kept, not stripped — see that record's `corners/matrix.md`) | 1 (output band), 4/5 (Kvco bound / band-selection rule) | **`RECORD-001` (#97): output band and Kvco-vs-band-code table bounded by a real PVT-cornered open-loop sweep (3 corner bundles x 4 band codes x 5 `VCTRL` points, 60/60 valid).** Band **445.26–1561.97 MHz**; Kvco avg **144.2–422.4 MHz/V**, Kvco peak local **162.2–379.9 MHz/V**; band select inert at `VCTRL=0.3V`, up to ~2.8x spread at `VCTRL=2.7V` (same shape as the SG13CMOS5L sibling). **Every one of the 60 measured frequencies is byte-for-byte identical to `sg13cmos5l-vco-kvco-table/records/RECORD-001`'s own table** — expected, since both designs' active devices (`sg13_hv_nmos`/`sg13_hv_pmos`) and corner-model files are literally shared between the two PDK trees, and the one per-instance difference (`XCDECAP`'s `cap_cmim`/`cap_cmomi` decap) is provably invisible to an ideal-voltage-source-driven measurement. **Confirms, rather than contradicts, `spec/decision-records/DR-005-fref-n-vco-band-reconciliation.md`'s own SG13CMOS5L-proxy assumption** — the SG13G2 band does not fall outside what DR-005 cited, so no follow-up decision record is needed. Row 3's top-of-band data point (1561.97 MHz) likewise matches DR-005's own 1562.0 MHz figure |
| [`sg13g2-divider-repair-reverification`](sg13g2-divider-repair-reverification/) | The non-port SG13G2 `divider_chain`, as repaired by #120 (the #112 repair ported verbatim to `design/dff_tg_hv.sch`): latch hold, exact commanded divide ratio, OP convergence, and the chain's own average supply current | 3 (multiplication ratio / retiming margin), 11 (power, `divider_chain` domain) | **`RECORD-001` (#120): the repair is confirmed on the SG13G2 PDK itself — DC OP converges (both ic modes; the broken as-drawn design never did), latch hold 9/9 corners at the rail, and the divide ratio is exact at every freshly simulated point (`N=64.000` at the `mos_tt`/`mos_ss`/`mos_ff` speed bracket; `N` = 65.000/96.000/127.000 structural bracket at nominal), with `idd` −218.6…−237.7 µA inside the port record's own 9-corner range.** The DUT netlist, every model card, and every OSDI object the all-MOS DUT loads are **byte-identical** to the port campaign's already-swept set (three `cmp`/`diff` checks quoted in the record), so the port RECORD-003 numbers for the stages not freshly re-run (remaining PVT points, remaining code words, tolerance arms, `setup`/`retime`) transfer as the same simulation — the same cross-PDK identity pattern `sg13g2-vco-kvco-table` established. Row 3's retiming-margin numeric sub-bound stays open exactly as the port record left it; row 3's functional-`N` half is now closed for the SG13G2 tree too |
