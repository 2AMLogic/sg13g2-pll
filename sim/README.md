...
**Deferred to follow-up issue #27** (Part of #16): the Kvco-vs-band-code
table (row 4/5), the divider retiming-margin closure (row 3), the Icp-trim
table and the actual loop-bandwidth/phase-margin numbers that combine it
with the filter data above (row 6/6a), lock time (row 7), duty cycle (row
13), lock-detector window/hysteresis (row 16), reference spur (row 10), and
power (row 11) — each needs either a closed-loop testbench, a long
transient/tuning-range sweep, or both, none of which this record's
per-device characterization methodology reaches. None of these rows is
