...
| 2 | Reference input | gf180-pll: 1–25 MHz CMOS square wave, rising-edge trigger, 30–70% duty (`pll.md` row 2, partly budget) | **Port the interface contract's *shape* (CMOS square wave, rising-edge trigger, 30–70% duty) as-is; the ported *frequency range* is amended by `DR-005` from 1–25 MHz to ≈3.5–24.4 MHz** — the measured SG13CMOS5L VCO band (`sim/sg13cmos5l-vco-kvco-table/`, 445.3–1562.0 MHz) cannot be reached by any allowed `N` (row 3) below this floor; re-derive the electrical levels once the supply flavor is settled (V_IL/V_IH scale with VDD) |
| 3 | Multiplication ratio | gf180-pll: N = 4–64, every integer (measured, `pll.md` row 3) | **Port the *requirement's structure* (a contiguous, no-holes integer range) as-is; the ported *range* is amended by `DR-005` from N ⊇ 4–64 to N ⊇ 64–127**, matching the as-drawn 6-cell ÷2/3 chain's own structural range (`N = 2^6 + Σp_i·2^i`) — the measured VCO band's floor (445.3 MHz) makes N = 4 unreachable at any credible reference frequency (would need f_ref ≥ 111 MHz) regardless of divider architecture, so gf180-pll `DR-001` Decision 3's own N = 4 sizing target does not transfer to SG13G2/SG13CMOS5L. **Functional confirmation measured (#112, `sim/sg13cmos5l-divider-nrange-retiming/records/RECORD-003`)**: after the dff hold-path repair, all 20 simulated points (all 9 PVT corners at N=64, every program-bit weight, two mixed codes, and N=127 at both speed-bracket extremes, 100 MHz) divide at exactly their commanded N. **Retiming-margin closure at the 1562.0 MHz top-of-band remains `unmet` for the provisional sizing (same record, Finding 4)**: one-edge capture fails everywhere (flop `t_setup` 0.5–1.5 ns ≥ 0.8–2.3 × `T_vco`); measured one-edge closing bound ≈ 825–1277 MHz by corner; above that the retimed `FB` latches one VCO period late (stable, constant extra loop delay). Re-closure is owed to the device-characterization sizing campaign (DR-001 T1 items 8–9), not to a spec change — this row is not relaxed to pass |
