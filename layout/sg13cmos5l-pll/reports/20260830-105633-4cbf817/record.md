# PLL SG13CMOS5L layout record — `20260830-105633-4cbf817`

- **klt**: `klt 0.3.0+gfdf04f71ab39`
- **PDK variant requested**: `ihp-sg13cmos5l`
- **PDK resolved**: not resolved (plan-only run)
...
## Verdict: **479 / 484 devices drawn**, **479 / 484 DRC-clean**, **479 / 484 re-extracted matching the schematic**; 6 / 6 blocks composed and routed (253 nets), 6 DRC-clean, 6 device-count-matched, **3 / 6 LVS `match`**

Drawn *and routed* by this repo's own `cmos5l_devices.py` / `cmos5l_route.py` — at an earlier pin, every `klt gen` generator and `klt gen-compose`'s router rejected the `ihp-sg13cmos5l` PDK family (klayout-tools#1462); **this run's own re-probe (below) shows that gap fixed at the current pin**, and this run *also* re-measured `klt gen-compose`'s router against a real multi-net block (`cp`, 13 multi-pin nets), where it routes **1 of 13** nets — klayout-tools#1467, reproduced at this pin, and the reason this flow keeps drawing and routing via this repo's own `cmos5l_devices.py`/`cmos5l_route.py`. **Verified entirely by `klt`**: `klt drc --deck sg13cmos5l`, `klt extract --deck sg13cmos5l --pdk ihp-sg13cmos5l` and `klt lvs`. Every device not drawn is a recorded, tracked upstream gap — see `layout/sg13cmos5l-pll/README.md`'s friction log, never a silent drop.
...
The two-pad probe above is far too small to exercise the finding that actually decides whether this flow's own router can be retired. So `klt gen-compose` is *also* re-probed every run against **`cp`**, this design's smallest composed block — 8 already-drawn group cells, 14 devices, 50 declared ports and 13 multi-pin `connectivity[]` nets, all taken straight from this run's own `plan.json` port→net map and drawn group geometry (never a synthetic case). Raw requests and responses are `gen-compose.probe.block-*.json`:

- declare-only (no `routing`): exit 3, 13 nets validated
- with `routing` (`layer_role: "metal"`): exit 3 — **1 of 13 nets routed**
- with a second routing plane (`routing.cross_block_layer_role`): exit 1 — `routing.cross_block_layer_role 'metal2' is not a known layer role for PDK family 'sg13cmos5l' -- available: active, contact, metal, metal_label, poly, res_mark, well, well_label`

Per-leg rejection reasons on the routed attempt, most frequent first (the full strings are in the committed response):

- 24 × `crosses already-routed net 'DN'`
- 6 × `backbone's 0.3um-wide drawn path crosses 7.43um through its own pin's block 'cp_nfet_w2_l0p5'`
- 4 × `backbone's 0.3um-wide drawn path crosses 3.35um through its own pin's block 'cp_nfet_w2_l0p5'`
- 4 × `backbone's 0.3um-wide drawn path crosses 5.47um through its own pin's block 'cp_nfet_w4_l1'`
- 4 × `backbone's 0.3um-wide drawn path crosses 7.31um through its own pin's block 'cp_nfet_w2_l0p5'`
- 3 × `backbone's 0.3um-wide drawn path crosses 7.79um through its own pin's block 'cp_nfet_w6_l0p5'`
...
| `mos_array` (`mos_pfet`) | `cp_pfet_w5_l0p5` | yes | clean | `pfet`×2 → `sg13_lv_pmos` | 0.46 µm ❌ under 0.6 µm | **no** | 2 |
| `res_array` (`res`) | `loop_filter_resrppd_w4_l120` | yes | clean | `rppd`×1 | 120.42 µm ✅ | **no** | 0 |
