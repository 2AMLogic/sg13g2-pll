# PLL SG13CMOS5L layout record — `20260921-155747-c44fa68`
...
## Verdict: **485 / 490 devices drawn**, **485 / 490 DRC-clean**, **485 / 490 re-extracted matching the schematic**; 6 / 6 blocks composed and routed (255 nets), 6 DRC-clean, 6 device-count-matched, **3 / 6 LVS `match`**
...
| `lock_detector` | 6/8 | 38 | 40 | 38 | 38 | yes | 23 (3 incomplete) | clean | yes | not converted |

### Floorplan

- `vco` is composed through the locality floorplan pass (issue #101): total routed wire **3912.55 um** vs **7177.91 um** for the same groups in the pre-#101 single-row order with by-name tracks (of which the track axis alone is worth `6808.91 um`). Per-net lengths, the chosen group order, the member->slot assignment and the track order are in `compose.vco.json` -> `floorplan`.
...
- 39 × `crosses already-routed net 'DN'`
- 9 × `backbone's 0.3um-wide drawn path crosses 7.43um through its own pin's block 'cp_nfet_w2_l0p5'`
- 6 × `backbone's 0.3um-wide drawn path crosses 13.78um through its own pin's block 'cp_pfet_w5_l0p5'`
- 5 × `backbone's 0.3um-wide drawn path crosses 15.78um through its own pin's block 'cp_pfet_w6_l0p3'`
- 5 × `backbone's 0.3um-wide drawn path crosses 3.35um through its own pin's block 'cp_nfet_w2_l0p5'`
- 5 × `backbone's 0.3um-wide drawn path crosses 7.31um through its own pin's block 'cp_nfet_w2_l0p5'`
...
| `pfd` | 202 | 37 | 8650.82 | — |
| `cp` | 70 | 18 | 2336.15 | — |
| `loop_filter` | 2 | 2 | 5.6 | `NZ`, `VCTRL`, `VSS` |
| `vco` | 137 | 33 | 3912.55 | `GND_VCO`, `VDD_VCO` |
| `divider_chain` | 950 | 142 | 147145.24 | — |
| `lock_detector` | 118 | 23 | 5889.96 | `ERRD`, `VSS`, `VWIN` |
