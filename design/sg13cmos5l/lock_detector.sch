...
Issue #66 (Part of #16) fixed BOTH of the two terms the paragraph above names, and row 16's hysteresis criterion now passes. (a) schmitt_hv.sch's two feedback devices were re-tied to the classic connection (see that schematic's own header); measured input-referred hysteresis 0.89-1.55 mV -> 879-979 mV over mos_tt/ff/ss x -40/27/125C. (b) XMPD was re-sized from w=2u l=0.5u to w=0.25u l=16u -- L/W 0.25 -> 64, i.e. ~256x its channel resistance -- because the settled VWIN is the balance between XRPU's charge over one reference period and XMPD's discharge over one WIDE pulse, so the XRPU/XMPD strength ratio, NOT R*C, sets how WIDE in phase error the VWIN transition is, and therefore how much phase-error hysteresis a given Schmitt voltage hysteresis buys. XRPU, XCW and XDW.XC1 are UNTOUCHED by #66, so #52's R*C margin (8.0-19.5x T_ref at the binding 3.5 MHz end) and its 3.688-11.24 ns assert window are spent by nothing here and were both re-measured to confirm it.
XMPD sizing rationale, both bounds measured (sim/sg13cmos5l-lock-detector-window/corners/xmpd_sizing.csv): weaker XMPD widens the transition and therefore RAISES hysteresis, but also pushes the assert/de-assert thresholds further out in phase error, and this topology's thresholds scale with T_ref (the charge-up time per discharge event), so the fast end of row 2's f_ref range is the binding one for hysteresis and the slow end is binding for threshold reach. w=0.25u l=16u is the sizing that clears BOTH: hysteresis 0.50x window at the strongest-discharge corner (mos_ff/res_wcs/-40C, 24.4 MHz -- 2.0x row 16's >= 0.25x criterion), and de-assert still inside the measured ladder at the weakest-discharge corner (mos_tt/res_bcs/125C, 3.5 MHz -- 18.2x window = 51% of T_ref). Sizings 1.4x stronger fall to 1.49x the criterion; sizings 2x weaker push de-assert past half a reference period. See sim/sg13cmos5l-lock-detector-window/records/RECORD-003-hysteresis-fix.md.
}
G {}
K {}
V {}
...
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 1300 -100 0 0 {name=MPD model=sg13_hv_nmos w=0.25u l=16u ng=1 m=1 spiceprefix=X}
