...
design/README.md.
Issue #120 fix (port of #112's repair, merged there via PR #121): each
latch's hold-path feedback is now two inversions, not one. Before this
change the master loop was M -> XIM -> MB -> XTGFBM -> M and the slave
loop S -> XIS -> SB -> XTGFBS -> S -- one inversion each, so a closed
feedback loop drove its own storage node to the inverter trip point
(a self-biased inverter, not a latch). The same topology defect in the
SG13CMOS5L port was bounded non-functional at every tested corner (0/9
hold, chain dead at 7/9 corners) by
sim/sg13cmos5l-divider-nrange-retiming RECORD-001 Finding 2, and this
exact repair was verified there by RECORD-003 (9/9 hold at the rail,
exact divide ratios across the PVT matrix). XIMF/XISF insert the missing
second inverter so each loop is non-inverting (bistable), matching the
proven fleet topology (gf180-pll dff_tg_3v3: XIA->XIB before XTGMF,
XIC->XID before XTGSF, all one standard inverter strength).
Issue #120 fix, part 2: XICKBB adds the second local clock phase (CKBB =
...
input XTG2: NG=CKBB/PG=CLKB, slave feedback XTGFBS: NG=CLKB/PG=CKBB.
Before this change all four TGs were clocked from the single-delay
CLK/CLKB pair, so the master feedback engaged at the same instant the
write path was still closing and the slave sampled the master
mid-contention -- the master-slave race the fleet topology's staggered
phases exist to prevent ("the master input gate closes one inverter
delay before the slave input gate opens"). The circuit edits are a
verbatim port of design/sg13cmos5l/dff_tg_hv.sch after #112 (instance
names, coordinates, and net labels identical).
